...
class TestGitpod:
    client = Gitpod(base_url=base_url, auth_token=auth_token, _strict_response_validation=True)
    ...
    def test_base_url_env(self) -> None:
        with update_env(GITPOD_BASE_URL="http://localhost:5000/from/env"):
            client = Gitpod(auth_token=auth_token, _strict_response_validation=True)
    ...
    def test_received_text_for_expected_json(self, respx_mock: MockRouter) -> None:
        ...
        strict_client = Gitpod(base_url=base_url, auth_token=auth_token, _strict_response_validation=True)

        with pytest.raises(APIResponseValidationError):
            strict_client.get("/foo", cast_to=Model)

        client = Gitpod(base_url=base_url, auth_token=auth_token, _strict_response_validation=False)
...
class TestAsyncGitpod:
    client = AsyncGitpod(base_url=base_url, auth_token=auth_token, _strict_response_validation=True)
    ...
    def test_base_url_env(self) -> None:
        with update_env(GITPOD_BASE_URL="http://localhost:5000/from/env"):
            client = AsyncGitpod(auth_token=auth_token, _strict_response_validation=True)
            assert client.base_url == "http://localhost:5000/from/env/"
    ...
    async def test_received_text_for_expected_json(self, respx_mock: MockRouter) -> None:
        ...
        strict_client = AsyncGitpod(base_url=base_url, auth_token=auth_token, _strict_response_validation=True)

        with pytest.raises(APIResponseValidationError):
            await strict_client.get("/foo", cast_to=Model)

        client = AsyncGitpod(base_url=base_url, auth_token=auth_token, _strict_response_validation=False)
